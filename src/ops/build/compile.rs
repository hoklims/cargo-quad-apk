...
use cargo::core::compiler::{CompileKind, CompileMode, CompileTarget, UserIntent};
...
use cargo_util::{ProcessBuilder, paths::dylib_path};
use clap::ArgMatches;
...
pub fn build_shared_libraries(
    ...
    for &build_target in config.build_targets.iter() {
        ...
        //
        // SAFETY: `std::env::set_var` is `unsafe` as of the 2024 edition because
        // mutating the process environment races with any thread that reads it.
        // Each batch of `set_var` here runs to completion before the
        // `compile_with_exec` call below; that call is synchronous and joins every
        // worker thread it spawns before it returns, so no other thread is alive to
        // read the environment while we mutate it. Do NOT move these calls into a
        // context where cargo's build threads are already running.
        unsafe {
            ...
            let cmake_toolchain_path =
                write_cmake_toolchain(config, &build_target_dir, build_target)?;
        ...
        // `compile_options` takes the high-level `UserIntent` (since cargo 0.95;
        // it was `CompileMode` in older versions). `CompileMode` is still used
        // below in the `Executor` impl, which operates at the lower level.
